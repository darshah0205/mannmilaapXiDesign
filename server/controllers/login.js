...
const login = async (req, res) => {
  try {
    ...
    if (validPassword) {
      ...
      return res.status(200).json({ success: true, token: token, msg: `Logged in as ${email}` });
    }
    else {
...
const authRoute = async (req, res) => {
  ...
  try {
    ...
    const user = await Client.findOne({ _id: decoded._id }).populate("groups", "name");

    if (!user || !user.isApproved) {
      return res.status(500).json({ success: false, msg: "User doesn't Exist or Not Approved" });
    }
    else {
      ...
      return res.status(200).json({ success: true, user: details, msg: `User Details` });
    }
